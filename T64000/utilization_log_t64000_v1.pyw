...
class Report(qtw.QWidget, Ui_Form):
    ...
    def write_data(self):
        ...
        if self.ts1500_checkBox.isChecked():
           linkam.append(self.ts1500_checkBox.text())
        if self.thms600_checkBox.isChecked():
           linkam.append(self.thms600_checkBox.text())
        linkam.append(self.linkam_startT_lineEdit.text() + ' - ' + self.linkam_stopT_lineEdit.text())
        #linkam.append(self.gases_textEdit.toPlainText())
